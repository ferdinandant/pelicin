...
These store a single human language character.
...
Defined in `<cstddef>` (`#include <cstddef>`), this type represents the size of an object in bytes. (But you can think of it as an unsigned integer.)
