...
The name of the variable should ONLY consist of letters (A&ndash;Z, a&ndash;z), numbers (0&ndash;9), and/or underscores (`_`), AND it must begin with a letter or an underscore.
...
Variables defined inside a function or inside a compound statement block (`{...}`) are _local variables_. You can access local variables in their scope only (inside their function or their nearest enclosing block).
...
Variables defined outside a function are _global variables_. You can access global variables anywhere inside the current file.
...
However, if the same name occurs in different nesting level, then you will use the variable closer to the current scope. (This behavior is known as _variable shadowing_, because the nested variable declaration "hides" the outer variable declaration.)
