...
Type casting explicitly converts one type into another type (Lospinoso).
...
`static_cast<type>(expression)` converts the value of the given `expression` to the given `type`.
...
int main() {
  ...
  // it will return the null/zero pointer
