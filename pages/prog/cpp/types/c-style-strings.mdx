...
Quote them using double quotes (`"`), e.g. `"Hello"`.

If the type is not array of `char`, don't forget to append them with the correct prefix:

- **`u`** &mdash; to make it an array of `char16_t`, e.g. `u"Hello"`
- **`U`** &mdash; to make it an array of `char32_t`, e.g. `U"Hello"`
- **`L`** &mdash; to make it an array of `wchar_t`, e.g. `L"Hello"`
