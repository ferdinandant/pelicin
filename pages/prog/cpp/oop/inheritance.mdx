...
A type that expects a superclass-type will also accept that superclass' subclasses ("a parent-type will accept its children-types"), but the inverse is not true. This works with pointers and references, too.

When you are using the superclass' type this way, you can only access the superclass' members through the variable, and not the subclasses'.
...
// This works fine because `StudentUser` is a `User`
User user = { "Eve" };
StudentUser student = { "Alice", 99 };
