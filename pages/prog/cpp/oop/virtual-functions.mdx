...
int main() {
  ...
  Base* base_casted_ptr = &derived;
  Base& base_casted_ref = derived;
  // The resolving behavior works with pointers and references:
  base_casted_ptr->normalFunction(); //=> Base: normal
  base_casted_ptr->virtualFunction(); //=> Derived: virtual (surprise!)
  base_casted_ref.normalFunction(); //=> Base: normal
  base_casted_ref.virtualFunction(); //=> Derived: virtual (surprise!)
}
```
...
Remember that within member function, references to other member function or variables are resolved via the implicit `this` pointer, which always points to the current class.
