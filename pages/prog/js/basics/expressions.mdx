...
This simply assigns the right-hand side expression to a variable or an object property (e.g. an array element). The return value of an assignment expression is the right-hand side value.
...
The `typeof` operator returns the "type" of its operand, which is one of: `number`, `boolean`, `string`, `symbol`, `object`, `function`, or `bigint`. Note that `typeof null` &rArr; `'object'` ("null is a special object").
