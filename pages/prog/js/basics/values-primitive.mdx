...
Primitive type includes `number`, `string`, `boolean`, `Symbol`, `null`, and `undefined`. Anything that is not primitive-type is object-type (described latter).

Properties of primitive values:

- **immutable** &mdash; they cannot be changed once they are created. To assign a new primitive value to a variable, you just simply create a new one.
