...
Codes are used to put remarks/explanations on codes. They are ignored by JavaScript.
...
Use `/**` and `*/` to insert a [JSDOC](https://jsdoc.app/) comment (this behaves like a multi-line comment, but your editor may parse this information and aids you during programming, e.g. by providing you with type hints and/or better autocomplete):
...
_Identifiers_ (names for your variables, functions, etc.) must begin with a letter, an underscore (`_`), or a dollar sign (`$`). Its subsequent characters may be letters, digits, underscores, and/or dollar signs.
...
These might also be used in future versions:
...
You can use Unicode characters in your identifier (as well as in strings and comments).

To escape them using ASCII characters, you can write `\uXXXX` or `\u{XXXX}`. (The version with curly braces is introduced in ES6 "to better support Unicode codepoints that require more than 16 bits," Flanagan.)
...
JavaScript does not force you to put semicolon (`;`) at the end of every statements. It has what is called [Automatic Semicolon Insertion (ASI)](https://en.wikibooks.org/wiki/JavaScript/Automatic_semicolon_insertion).
