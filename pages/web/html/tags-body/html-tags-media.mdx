import { References, HTMLTagsIndexTable, NoteBox, WebEditor, Badge } from '@pelicin/components';
...
<NoteBox type="info">
  <p>The essential things on this page are:</p>
  <ul>
    <li>
      <code>{`<img>`}</code>, <code>{`<audio>`}</code>, <code>{`<video>`}</code> to embed multimedia
      files
    </li>
    <li>
      <code>{`<iframe>`}</code> to embed HTML pages
    </li>
  </ul>
</NoteBox>
...
<ul>
  ...
  <li>
    <Attribute>srcset=[...]</Attribute> &mdash;{' '}
    <a href="/web/html/attributes/html-attributes-recurring#srcset">read here</a>
  </li>
  <li>
    <Attribute>sizes=[...]</Attribute> &mdash;{' '}
    <a href="/web/html/attributes/html-attributes-recurring#sizes">read here</a>
  </li>
</ul>
...
<WebEditor
  html={smartTrim(`
    <!-- Image source: -->
    <!-- https://developer.mozilla.org/en-US/docs/Web/HTML/Element/area -->
    <embed src="/res/web/html/mdn-info.png" type="image/png" width="150">
  `)}
/>
...
<WebEditor
  html={smartTrim(`
    <iframe width="200" src="https://www.youtube-nocookie.com/embed/gN24W_psMpE" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
  `)}
/>
...
<References
  references={[
    ...
    {
      text: 'Top tips for web performance: Use srcset to automatically choose the right image size (web.dev)',
