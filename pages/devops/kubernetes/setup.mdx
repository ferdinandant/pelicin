...
### 2. Ensure that your image is accesible to the Minikube cluster

If you don't push your image to the Docker registry, load your image to Minikube:
...
If you don't do this, Kubernetes won't be able to pull/load the image when you run `kubectl run` (it tries to pull the image from the registry).

### 3. Deploying your Docker image
...
<NoteBox>
  I have updated the command to not use <Code str="--generator=run/v1" /> from the book
  (deprecated).
</NoteBox>
...
The `--image` option specifies the image that you want to run, while `--port=8080` tells Kubernetes that your app is listening on port 8080
